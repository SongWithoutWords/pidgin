...
runConstrainM :: ConstrainM a -> Ast -> a
runConstrainM constrainM ast =
  let (x, _, _) = runRWS constrainM ast initialState
  in x
...
addLocalBinding (n, t) = modifyCurrentScope $ M.insert n $ KExpr $ Expr t $ EBinding
