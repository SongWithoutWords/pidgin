...
tests = testGroup "parser"
  ...
  , namedTest "def pi"
    "$ pi = 3.14159265"
    [("pi", UVar $ Var Imt Nothing $ EVal $ VFlt 3.14159265)]

  , namedTest "op expr"
    "$ three = 1 + 2"
    [ ("three", UVar $ Var Imt Nothing
  ...
  , namedTest "negate inline"
    ...
    [ ("negate", UFunc $ Func (Sig Pure [Param Imt TBln "b"] $ Just TBln)
      [ EIf (EName "b")
        [EVal $ VBln False]
        [EVal $ VBln True]
      ]
...
factorial(Int n) -> Int =>
    ...
    |]
    [ ("factorial", UFunc $ Func ( Sig Pure [Param Imt TInt "n"] $ Just TInt)
      [ EIf
...
drawWidget(~@, Nat width, Nat height) =>
    ...
    if w.exists then
        w.draw(~@)
    |]
    [ ("drawWidget", UFunc $ Func
      ( Sig PWrite [Param Imt TNat "width", Param Imt TNat "height"] $ Nothing )
      [ EVar ("w", Var Imt Nothing $ EApp (EName "Widget") Pure
...
|]
    [ ("f", UFunc $ Func
      (Sig Pure [] Nothing)
      [ EVar ("arr", Var Mut Nothing $ EApp (EName "Array") Pure
